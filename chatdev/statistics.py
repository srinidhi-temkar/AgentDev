...
def prompt_cost(model_type: str, num_prompt_tokens: float, num_completion_tokens: float):
    input_cost_map = {
        ...
        "gpt-4o": 0.005,
        "gpt-4o-mini": 0.00015,
        os.getenv("BASE_MODEL_NAME"): float(os.getenv("BASE_MODEL_COST")),
        os.getenv("FT_MODEL_NAME"): float(os.getenv("FT_MODEL_COST")),
    }
...
def get_info(dir, log_filepath):
    ...
    if os.path.exists(dir):
        ...
        if len(sublines) > 0:
            ...
            elif model_type == "CUSTOM_BASE_MODEL":
                model_type = os.getenv("BASE_MODEL_NAME")
            elif model_type == "CUSTOM_FT_MODEL":
                model_type = os.getenv("FT_MODEL_NAME")
